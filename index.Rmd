...
author: "Lic. Fernando Bermudez Kuminev"
date: "2020-09-16"
output:
  html_document:
    theme: readable    
...
```{r}
library(gcookbook)
paises_ca <- c(rep("PAN", 4), rep("CRI",4), rep("NIC",4), rep("SLV",4), rep("HND",4), rep("GTM",4), rep("BLZ",4), rep("DOM",4))
condicion <- rep(c("fallecidos", "activos", "positivos", "recuperados"),4)
valores_ca <- abs(c(ca_covid19$fallecidos, ca_covid19$activos, ca_covid19$positivos, ca_covid19$recuperados))
centro_america_dom <- data.frame(paises_ca,condicion,valores_ca)

ggplot(centro_america_dom, aes(fill=condicion, y=valores_ca, x=paises_ca)) + 
    geom_bar(position="dodge", stat="identity")+
  ggtitle("Comportamiento de casos en Centroamerica y Rep. Dominicana")+
  scale_fill_discrete(name = "Condicion", labels = c("Activos", "Fallecidos", "Positivos", "Recuperados"))+
 geom_abline(slope=0, intercept=2173,  col = "red",lty=2)
